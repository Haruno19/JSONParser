...
jsonaccess(JSONObject, FieldList, Result) :-
    %term_string(JSONObject, JSONObjectTemp),
    %string_chars(JSONObjectTemp, JSONObjectChars),
    format(chars(JSONObjectChars), "~w", JSONObject),
     %%tolgo la ')' finale che altrimenti sarebbe ridondante farlo in due volte mmmmmmmhhhhhhh?????????????????????????????
...
verificaOBJ(JSONObjectString, [Field | OtherFields], Result) :-
    ...
    check_LastChar(')', MembersCharListTemp, MembersCharListTemp1),
    check_LastChar(']', MembersCharListTemp1, MembersCharList),
    membersToList(MembersCharList, MemberList),
    ...
    extractObjectValue(MemberList, [Field | OtherFields], Result),
    !.
...
extractObjectValue([], _, _) :- !. %%brother in christ il cut non serve se c'e il false prima
...
extractObjectValue([Member | _], [Field | OtherFields], Result) :-
    ...
    string_chars(FieldCurrentTemp, FieldCurrentChars),
    %is_JSONField(FieldCurrentTemp, FieldCurrent),
    %string_chars(Field, FieldChars),
    %format(chars(FieldDef), "~w", FieldChars),
    normalize_space(string(NormField), Field),
    normalize_space(string(NormField), FieldCurrentTemp),
    %string_chars(ValueCurrentTemp, ValueCurrentChars),
    %is_JSONValue(ValueCurrentTemp, ValueCurrent),
    formatValue(ValueCurrentChars, Value),
    verificaOBJ(Value, OtherFields, Result), 
    !.

extractObjectValue([_ | OtherMembers], Fields, Result) :-
    extractObjectValue(OtherMembers, Fields, Result).


formatValue(CharList, Value) :-
    string_chars(String, CharList),
    catch(is_JSONumber(String, Value), error(syntax_error(_), _), false),
    !.

formatValue(CharList, String) :-
    string_chars(String, CharList).
